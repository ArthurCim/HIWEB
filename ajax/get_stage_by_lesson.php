    ...
    // Get quiz details if type is quiz
        ...
        try {
            $quiz_stmt = $conn->prepare(
                "SELECT q.id_question, q.content AS question_text, q.id_stage, q.answers_type, 
                        qo.id_question_option, qo.option_text, qo.is_correct 
                 FROM question q 
                ...
                while ($q = $quiz_res->fetch_assoc()) {
                    if (!isset($quiz_details[$q['id_question']])) {
                        $quiz_details[$q['id_question']] = [
                            ...
                            'isi' => $q['question_text'],
                            'quiz_type' => $q['answers_type'] ?? 'pilihan_ganda',
                            'options' => []
                        ];
                    }
